...
func (c *Controller) SetInitialBlocksRange(chainIDs []uint64) error {
	...
	// 	return err
	// }
	chainClients := make(map[uint64]*chain.ClientWithFallback)
	chainClients[1] = c.chainClient
...
func (c *Controller) CheckRecentHistory(chainIDs []uint64, accounts []common.Address) error {
	...
	// c.strategy.setChainClients(chainClients)
	// c.strategy.setAccounts(accounts)

	if c.reactor != nil {
		err := c.reactor.restart(chainClients, accounts, c.fetchStrategyType)
		// err := c.reactor.restart(c.strategy)
		if err != nil {
			return err
		}
	} else {
