...
func NewBackoffConnector(h host.Host, cacheSize int, connectionTryDuration time.Duration, backoff backoff.BackoffFactory, logger *zap.Logger) (*BackoffConnector, error) {
	...
	return &BackoffConnector{
		...
		backoff:    backoff,
		logger:     logger.Named("backoff-connector"),
	}, nil
}
